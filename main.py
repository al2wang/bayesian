...
class ActiveLearningExperiment:
    def __init__(self, config):
        ...
        self.target = TARGETS_DICT[self.cfg["target_name"]](dim=self.dim, domain=self.bounds)

        n_initial = 50
        self.X_train = torch.rand(n_initial, self.dim) * (self.bounds[1] - self.bounds[0]) + self.bounds[0]
        self.y_train = self.target(self.X_train) + self.cfg['noise_var'] * torch.randn(n_initial, 1)
        self.history = []
        ...
        self.y_gt = self.target(X_gt).detach().numpy().ravel()
    ...
    def run(self):
        ...
        self._setup_grid()    
        self.target_energies = self.target(self.X_grid)
        ...
        for i in range(self.cfg['n_iterations']):
            ...
            y_next_true = self.target(x_next)
    ...
    def _step_grid(self, iter_idx):
        return self._step_with_candidates(iter_idx, self.X_grid.reshape(-1, self.cfg["dim"]))
    ...
    def _get_Z(self):
        ...
        if self.X_grid == None:
            ...
            self._setup_grid()
        energy = self.target(self.X_grid.reshape(-1, self.cfg["dim"]))  # NOTE: this function only works with vectors
        logits = -energy / self.cfg['temperature']
    ...
    def _step_uniform(self, iter_idx):
        candidates = self.target.sample_domain_uniform(num=self.cfg["n_candidates"])
    ...
    def plot_experiment(self, data_path, output_folder):
        ...
        if 'X_grid' in history[0]:
            ...
            plt.fill_between(X_grid.view(-1).numpy(), 
                            ...
                            (mu + 2*std).view(-1).numpy(), 
                            color='blue', alpha=0.2, label="uncertainty")
            plt.scatter(X_train_final.numpy(), self.target(X_train_final).numpy(), c='k', marker='x', label="all samples")
        ...
        y_gt = self.target(X_gt).numpy().ravel()
...
def main(
    ...
    config = {
        ...
        'noise_var': noise_var,
        'energy_mode': energy_mode, # TODO: rename to acquisi. func. to test botorch mode
